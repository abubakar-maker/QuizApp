...
function MyResult({ apiBase = API_BASE }) {
  ...
  useEffect(() => {
  if (!results) return;
  const techSet = new Set(results.map(r => r.technology));
  setTechnologies([...techSet].sort());
}, [results]);
        ...
        <div className={resultStyles.filterContainer}>
          <div className={resultStyles.filterContent}>
            <div className={resultStyles.filterButtons}>
              ...
              <button onClick={()=> handleSelectTech("all")} className={`${resultStyles.filterButton} ${selectedTechnology === 'all' ? resultStyles.filterButtonActive : resultStyles.filterButtonInactive}`}>
                All
              </button>
            ...
            <div className={resultStyles.filterStatus}>
              {selectedTechnology === 'all' ? 'Showing all the tecnologies ' : `filtering: ${selectedTechnology}`}
            </div>
          </div>
        </div>
        {loading ? (
          ...
          {Array.isArray(results) && results.length === 0 && !error && (
            <div className={resultStyles.emptyState}>
              No result yet. take  a quiz to see your result
            </div>
          )}
          </>
        )}
        ...
        <div className={resultStyles.cardInfo}>
          ...
          <div className={resultStyles.cardText}>
            ...
            <div className={resultStyles.cardMeta}>
              {item.totalQuestions} QS {item.timeSpent ? ` • ${item.timeSpent}` : ""}
            </div>
          </div>
        </div>
      </div>
